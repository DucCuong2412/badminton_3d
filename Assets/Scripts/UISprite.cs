...
public class UISprite : UIWidget
{
	...
	public override Vector4 drawingDimensions
	{
		get
		{
			...
			Vector4 vector = border * atlas.pixelSize;
	...
	protected void TiledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (; num < drawingDimensions.w; num += a.y)
		{
			...
			{
				y = Mathf.Lerp(mInnerUV.yMin, mInnerUV.yMax, (drawingDimensions.w - num) / a.y);
			...
			for (; x < drawingDimensions.z; x += a.x)
			{
				...
				{
					x2 = Mathf.Lerp(mInnerUV.xMin, mInnerUV.xMax, (drawingDimensions.z - x) / a.x);
	...
	protected void AdvancedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Color color = base.color;
		color.a = finalAlpha;
		Color32 c = (!atlas.premultipliedAlpha) ? color : NGUITools.ApplyPMA(color);
		for (int i = 0; i < 3; i++)
		{
			int num = i + 1;
			for (int j = 0; j < 3; j++)
					...
					if (centerType == AdvancedType.Tiled)
					{
						...
						for (float num3 = y; num3 < y2; num3 += a.y)
						{
							...
							for (; num4 < x2; num4 += a.x)
							{
								...
								FillBuffers(num4, num7, num3, num6, x3, num8, y3, num5, c, verts, uvs, cols);
							}
						}
					}
					else if (centerType == AdvancedType.Sliced)
					{
						FillBuffers(mTempPos[i].x, mTempPos[num].x, mTempPos[j].y, mTempPos[num2].y, mTempUVs[i].x, mTempUVs[num].x, mTempUVs[j].y, mTempUVs[num2].y, c, verts, uvs, cols);
					}
				}
				else if (i == 1)
				{
					if ((j == 0 && bottomType == AdvancedType.Tiled) || (j == 2 && topType == AdvancedType.Tiled))
					{
						...
						for (float num9 = x4; num9 < x5; num9 += a.x)
						{
							...
							FillBuffers(num9, num10, y4, y5, x6, num11, y6, y7, c, verts, uvs, cols);
						}
					}
				...
				else if (j == 1)
				{
					if ((i == 0 && leftType == AdvancedType.Tiled) || (i == 2 && rightType == AdvancedType.Tiled))
					{
						...
						for (float num12 = y8; num12 < y9; num12 += a.y)
						{
							...
							FillBuffers(x7, x8, num12, num14, x9, x10, y10, num13, c, verts, uvs, cols);
						}
					}
