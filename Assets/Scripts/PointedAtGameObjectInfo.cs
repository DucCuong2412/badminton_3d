	...
	private void OnGUI()
	{
		if (InputToEvent.goPointedAt != null)
		{
			...
			if (photonView != null)
			{
				...
				GUI.Label(new Rect(x, num - mousePosition2.y - 15f, 300f, 30f), string.Format("ViewID {0} InstID {1} Lvl {2} {3}", photonView.viewID, photonView.instantiationId, photonView.prefix, photonView.isSceneView ? "scene" : ((!photonView.isMine) ? ("owner: " + photonView.ownerId) : "mine")));
			}
		}
	}
}
