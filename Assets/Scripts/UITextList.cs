...
public class UITextList : MonoBehaviour
{
	...
	protected int scrollHeight
	{
		get
		{
			...
			int num = Mathf.FloorToInt((float)textLabel.height / (float)textLabel.fontSize);
	...
	protected void Rebuild()
	{
		...
		for (int i = 0; i < mParagraphs.size; i++)
		{
			...
			if (NGUIText.WrapText(paragraph.text, out string finalText))
			{
		...
		if (scrollBar != null)
		{
			...
			if (uIScrollBar != null)
			{
				uIScrollBar.barSize = 1f - (float)scrollHeight / (float)mTotalLines;
			}
		}
		UpdateVisibleText();
	}
