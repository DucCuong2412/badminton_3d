...
public class NGUIJson
{
	...
	[CompilerGenerated]
	private static Comparison<UISpriteData> _003C_003Ef__mg_0024cache0;
	...
	public static void LoadSpriteData(UIAtlas atlas, TextAsset asset)
	{
		if (!(asset == null) && !(atlas == null))
		{
			...
			asset = null;
			LoadSpriteData(atlas, text);
		}
	}

	public static void LoadSpriteData(UIAtlas atlas, string jsonString)
	{
		if (!(atlas == null) && !string.IsNullOrEmpty(jsonString))
		{
			Hashtable hashtable = jsonDecode(jsonString) as Hashtable;
			if (hashtable == null)
			{
				UnityEngine.Debug.LogWarning("Unable to parse Json file ");
				return;
			}
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					uISpriteData.name = dictionaryEntry.Key.ToString();
					bool flag = false;
					foreach (UISpriteData item in spriteList)
					{
						if (item.name.Equals(uISpriteData.name, StringComparison.OrdinalIgnoreCase))
						{
							flag = true;
							break;
						}
					...
					if (!flag)
					{
						uISpriteData.name = uISpriteData.name.Replace(".png", string.Empty);
						uISpriteData.name = uISpriteData.name.Replace(".tga", string.Empty);
					}
					Hashtable hashtable3 = (Hashtable)dictionaryEntry.Value;
					Hashtable hashtable4 = (Hashtable)hashtable3["frame"];
					int x = int.Parse(hashtable4["x"].ToString());
					int y = int.Parse(hashtable4["y"].ToString());
					int num = int.Parse(hashtable4["w"].ToString());
					int num2 = int.Parse(hashtable4["h"].ToString());
					uISpriteData.x = x;
					uISpriteData.y = y;
					uISpriteData.width = num;
					uISpriteData.height = num2;
					...
					if (hashtable6 != null && hashtable5 != null)
					{
						if (num > 0)
						{
							int num3 = int.Parse(hashtable6["x"].ToString());
							int num4 = int.Parse(hashtable6["w"].ToString());
							int num5 = int.Parse(hashtable5["w"].ToString());
							uISpriteData.paddingLeft = num3;
							uISpriteData.paddingRight = num5 - (num3 + num4);
						}
						if (num2 > 0)
						{
							int num6 = int.Parse(hashtable6["y"].ToString());
							int num7 = int.Parse(hashtable6["h"].ToString());
							int num8 = int.Parse(hashtable5["h"].ToString());
							uISpriteData.paddingTop = num6;
							uISpriteData.paddingBottom = num8 - (num6 + num7);
						}
					}
					foreach (UISpriteData item2 in spriteList)
					{
						if (item2.name.Equals(uISpriteData.name, StringComparison.OrdinalIgnoreCase))
						{
							uISpriteData.borderLeft = item2.borderLeft;
							uISpriteData.borderRight = item2.borderRight;
							uISpriteData.borderBottom = item2.borderBottom;
							uISpriteData.borderTop = item2.borderTop;
						}
					}
	...
	private static int CompareSprites(UISpriteData a, UISpriteData b)
	{
