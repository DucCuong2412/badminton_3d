...
public static class NGUIMath
{
	...
	{
		return CalculateRelativeWidgetBounds(root, child, considerInactive: false);
	}
	...
	public static void ResizeWidget(UIWidget w, UIWidget.Pivot pivot, float x, float y, int minWidth, int minHeight)
	{
		...
		case UIWidget.Pivot.BottomLeft:
			AdjustWidget(w, point.x, point.y, 0f, 0f, minWidth, minHeight);
			break;
		case UIWidget.Pivot.Left:
			AdjustWidget(w, point.x, 0f, 0f, 0f, minWidth, minHeight);
			break;
		case UIWidget.Pivot.TopLeft:
			AdjustWidget(w, point.x, 0f, 0f, point.y, minWidth, minHeight);
			break;
		case UIWidget.Pivot.Top:
			AdjustWidget(w, 0f, 0f, 0f, point.y, minWidth, minHeight);
			break;
		case UIWidget.Pivot.TopRight:
			AdjustWidget(w, 0f, 0f, point.x, point.y, minWidth, minHeight);
			break;
		case UIWidget.Pivot.Right:
			AdjustWidget(w, 0f, 0f, point.x, 0f, minWidth, minHeight);
			break;
		case UIWidget.Pivot.BottomRight:
			AdjustWidget(w, 0f, point.y, point.x, 0f, minWidth, minHeight);
			break;
		case UIWidget.Pivot.Bottom:
			AdjustWidget(w, 0f, point.y, 0f, 0f, minWidth, minHeight);
			break;
		}
	}
	...
	public static void AdjustWidget(UIWidget w, float left, float bottom, float right, float top, int minWidth, int minHeight)
	{
		...
		if (num7 < num5)
		{
			if (num != 0)
			{
				zero2.x -= Mathf.Lerp(num5 - num7, 0f, pivotOffset.x);
			}
			else
			{
				zero2.x += Mathf.Lerp(0f, num5 - num7, pivotOffset.x);
			}
			num7 = num5;
		}
		if (num8 < num6)
		{
			if (num2 != 0)
			{
				zero2.y -= Mathf.Lerp(num6 - num8, 0f, pivotOffset.y);
			}
			else
			{
				zero2.y += Mathf.Lerp(0f, num6 - num8, pivotOffset.y);
			}
			num8 = num6;
		}
		if (num7 < minWidth)
		...
		if (w.isAnchored)
		{
			...
			float num9 = vector10.x - pivotOffset.x * (float)num7;
			float num10 = vector10.y - pivotOffset.y * (float)num8;
			if ((bool)w.leftAnchor.target)
			{
			...
			if ((bool)w.rightAnchor.target)
			{
				w.rightAnchor.SetHorizontal(cachedTransform, num9 + (float)num7);
			}
			if ((bool)w.bottomAnchor.target)
			{
			...
			if ((bool)w.topAnchor.target)
			{
				w.topAnchor.SetVertical(cachedTransform, num10 + (float)num8);
			}
		}
	}
}
