...
public class UICamera : MonoBehaviour
{
	...
	public static bool Raycast(Vector3 inPos, out RaycastHit hit)
	{
		for (int i = 0; i < list.size; i++)
		{
			...
			{
				if (uICamera.eventType != EventType.UI)
				{
					continue;
				}
	...
	private void Update()
	{
		...
		{
			if (cancelKey0 != 0 && UnityEngine.Input.GetKeyDown(cancelKey0))
			{
				currentScheme = ControlScheme.Controller;
				currentKey = cancelKey0;
				selectedObject = null;
			}
			else if (cancelKey1 != 0 && UnityEngine.Input.GetKeyDown(cancelKey1))
			{
				currentScheme = ControlScheme.Controller;
		...
		if (mCurrentSelection != null)
		{
			...
			if (useKeyboard && UnityEngine.Input.GetKeyDown(KeyCode.Delete))
			{
				text += "\b";
			}
			if (text.Length > 0)
		...
		{
			float num = string.IsNullOrEmpty(scrollAxisName) ? 0f : UnityEngine.Input.GetAxis(scrollAxisName);
			if (num != 0f)
			{
				Notify(mHover, "OnScroll", num);
			}
			if (showTooltips && mTooltipTime != 0f && (mTooltipTime < RealTime.time || UnityEngine.Input.GetKey(KeyCode.LeftShift) || UnityEngine.Input.GetKey(KeyCode.RightShift)))
			{
				mTooltip = mHover;
	...
	public void ProcessMouse()
	{
		...
		if (flag2 || flag || mNextRaycast < RealTime.time)
		{
			...
			if (!Raycast(UnityEngine.Input.mousePosition, out lastHit))
			{
	...
	public void ProcessTouches()
	{
		...
		if (UnityEngine.Input.touchCount == 0 && useMouse)
		{
			ProcessMouse();
		}
	}
	...
	public void ProcessOthers()
	{
		...
		bool flag = false;
		bool flag2 = false;
		if (submitKey0 != 0 && UnityEngine.Input.GetKeyDown(submitKey0))
		{
			currentKey = submitKey0;
			flag = true;
		}
		if (submitKey1 != 0 && UnityEngine.Input.GetKeyDown(submitKey1))
		{
			currentKey = submitKey1;
			flag = true;
		}
		if (submitKey0 != 0 && UnityEngine.Input.GetKeyUp(submitKey0))
		{
			currentKey = submitKey0;
			flag2 = true;
		}
		if (submitKey1 != 0 && UnityEngine.Input.GetKeyUp(submitKey1))
		{
			currentKey = submitKey1;
			flag2 = true;
		}
		...
		if (useKeyboard && UnityEngine.Input.GetKeyDown(KeyCode.Tab))
		{
		...
		if (cancelKey1 != 0 && UnityEngine.Input.GetKeyDown(cancelKey1))
		{
			currentKey = cancelKey1;
			currentScheme = ControlScheme.Controller;
			Notify(mCurrentSelection, "OnKey", KeyCode.Escape);
