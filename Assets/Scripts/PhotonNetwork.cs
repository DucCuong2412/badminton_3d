...
public static class PhotonNetwork
{
	...
	public static void SwitchToProtocol(ConnectionProtocol cp)
	{
		if (networkingPeer.UsedProtocol != cp)
		{
			...
			networkingPeer.LimitOfUnreliableCommands = 40;
			UnityEngine.Debug.Log("Protocol switched to: " + cp);
		}
	}
	...
	public static bool LeaveRoom()
	{
		...
		{
			UnityEngine.Debug.LogError("PhotonNetwork: Error, you cannot leave a room if you're not in a room! State: " + connectionStateDetailed);
			return false;
		}
		if (room == null)
		{
			UnityEngine.Debug.LogError("PhotonNetwork: Error, you cannot leave a room if you're not in a room! Room is null.");
			return false;
		}
		if (offlineMode)
		{
	...
	public static void UnAllocateViewID(int viewID)
	{
		...
		{
			UnityEngine.Debug.LogWarning($"Unallocated manually used viewID: {viewID} but found it used still in a PhotonView: {networkingPeer.photonViewList[viewID]}");
		}
	}
