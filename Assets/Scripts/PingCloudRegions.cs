...
public class PingCloudRegions : MonoBehaviour
{
	...
	public static bool ClosestRegionAvailable => !string.IsNullOrEmpty(ClosestRegion);
	...
	public static void RefreshCloudServerRating()
	{
		if (Instance != null)
		{
			...
			{
				UnityEngine.Debug.Log("RefreshCloudServerRating already in process. Skipping this call.");
			}
			else
			{
	...
	{
		if (!ClosestRegionAvailable || !ServerSettings.TryParseCloudServerRegion(ClosestRegion, out CloudServerRegion bestRegion))
		{
			RefreshCloudServerRating();
		}
		while (isPinging)
	...
	public static void DeleteRegionPref()
	{
		...
		{
			UnityEngine.Debug.LogWarning("DeleteRegionPref can't delete region while pining is going on.");
			return;
		}
		ClosestRegion = null;
