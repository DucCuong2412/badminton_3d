...
public class PhotonView : Photon.MonoBehaviour
{
	...
	public void OnDestroy()
	{
		...
		if (!destroyedByPhotonNetworkOrQuit && !Application.isLoadingLevel)
		{
			...
			{
				UnityEngine.Debug.LogError("OnDestroy() seems to be called without PhotonNetwork.Destroy()?! GameObject: " + base.gameObject + " Application.isLoadingLevel: " + Application.isLoadingLevel);
			}
			else if (viewID <= 0)
			{
				UnityEngine.Debug.LogWarning($"OnDestroy manually allocated PhotonView {this}. The viewID is 0. Was it ever (manually) set?");
			}
			else if (isMine && !PhotonNetwork.manuallyAllocatedViewIds.Contains(viewID))
			{
				UnityEngine.Debug.LogWarning($"OnDestroy manually allocated PhotonView {this}. The viewID is local (isMine) but not in manuallyAllocatedViewIds list. Use UnAllocateViewID() after you destroyed the PV.");
			}
		}
		if (PhotonNetwork.networkingPeer.instantiatedObjects.ContainsKey(instantiationId))
		{
			...
			if (flag)
			{
				UnityEngine.Debug.LogWarning(string.Format("OnDestroy for PhotonView {0} but GO is still in instantiatedObjects. instantiationId: {1}. Use PhotonNetwork.Destroy(). {2} Identical with this: {3} PN.Destroyed called for this PV: {4}", this, instantiationId, (!Application.isLoadingLevel) ? string.Empty : "Loading new scene caused this.", flag, destroyedByPhotonNetworkOrQuit));
			}
		}
	}
	...
	protected internal void ExecuteOnSerialize(PhotonStream pStream, PhotonMessageInfo info)
	{
		if (!failedToFindOnSerialize)
		{
			...
			{
				UnityEngine.Debug.LogError("The observed monobehaviour (" + observed.name + ") of this PhotonView does not implement OnPhotonSerializeView()!");
