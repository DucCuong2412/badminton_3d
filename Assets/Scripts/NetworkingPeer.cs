...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
	...
	public void ChangeLocalID(int newID)
	{
		if (mLocalActor == null)
		{
			UnityEngine.Debug.LogWarning($"Local actor is null or not in mActors! mLocalActor: {mLocalActor} mActors==null: {mActors == null} newID: {newID}");
		}
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		{
			UnityEngine.Debug.LogError("Operation " + operationResponse.OperationCode + " could not be executed yet. Wait for state JoinedLobby or ConnectedToMaster and their respective callbacks before calling OPs.");
		}
		...
		case 220:
			...
			UnityEngine.Debug.Log("GetRegions returned: " + operationResponse.ToStringFull());
			break;
		case 227:
		{
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 228:
			...
			{
				UnityEngine.Debug.LogError("Event QueueState must contain position!");
			}
			if (mQueuePosition == 0)
			{
				if (PhotonNetwork.autoJoinLobby)
		...
		case byte.MaxValue:
		{
			...
			if (num == mLocalActor.ID)
			{
				...
				foreach (int num5 in array2)
				{
					...
					{
						AddNewPlayer(num5, new PhotonPlayer(isLocal: false, num5, string.Empty));
					}
				}
		...
		case 207:
		{
			...
			if (num6 >= 0)
			{
				DestroyPlayerObjects(num6, localOnly: true);
				break;
			}
		...
		case 204:
		{
			...
			if (value == null || photonPlayer == null)
			{
				...
				{
					UnityEngine.Debug.LogError("Can't execute received Destroy request for view ID=" + num2 + " as GO can't be found. From player/actorNr: " + num + " GO to destroy=" + value + "  originating Player=" + photonPlayer);
				}
			}
			else
			{
	...
	public void ExecuteRPC(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender)
	{
		...
		if (photonView == null)
		{
			...
			if (flag)
			{
				UnityEngine.Debug.LogWarning("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! View was/is ours." + ((!flag2) ? " Remote called." : " Owner called."));
			}
			else
			{
				UnityEngine.Debug.LogError("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! Was remote PV." + ((!flag2) ? " Remote called." : " Owner called."));
			}
			return;
		}
		...
		if (num6 == 0)
		{
			if (num7 == 0)
			{
				UnityEngine.Debug.LogError("PhotonView with ID " + num + " has no method \"" + text + "\" marked with the [RPC](C#) or @RPC(JS) property! Args: " + text2);
			}
			else
			{
	...
	internal GameObject DoInstantiate(ExitGames.Client.Photon.Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
	{
		...
		{
			if (GetMethod(monoBehaviour, PhotonNetworkingMessage.OnPhotonInstantiate.ToString(), out MethodInfo mi))
			{
	...
	public PhotonView GetPhotonView(int viewID)
	{
		...
		if (value == null)
		{
			...
			foreach (PhotonView photonView in array2)
			{
				...
				{
					UnityEngine.Debug.LogWarning("Had to lookup view that wasn't in dict: " + photonView);
					return photonView;
				}
			}
		}
	...
	public void RegisterPhotonView(PhotonView netView)
	{
		...
		else
		{
			...
			if (photonViewList.ContainsKey(netView.viewID))
			{
				...
				{
					UnityEngine.Debug.LogError($"PhotonView ID duplicate found: {netView.viewID}. New: {netView} old: {photonViewList[netView.viewID]}. Maybe one wasn't destroyed on scene load?! Check for 'DontDestroyOnLoad'. Destroying old entry, adding new.");
				}
	...
	internal void RPC(PhotonView view, string methodName, PhotonPlayer player, params object[] parameters)
	{
		if (!blockSendingGroups.Contains(view.group))
		{
			...
			if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
			{
				UnityEngine.Debug.Log("Sending RPC \"" + methodName + "\" to player[" + player + "]");
			}
			ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable();
			hashtable[(byte)0] = view.viewID;
			if (view.prefix > 0)
	...
	internal void RPC(PhotonView view, string methodName, PhotonTargets target, params object[] parameters)
	{
		...
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
		{
			UnityEngine.Debug.Log("Sending RPC \"" + methodName + "\" to " + target);
		}
		ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable();
		hashtable[(byte)0] = view.viewID;
		if (view.prefix > 0)
	...
	protected internal void LoadLevelIfSynced()
	{
		...
		if (obj is int)
		{
			if (UnityEngine.SceneManagement.SceneManager.GetActiveScene().buildIndex != (int)obj)
			{
				PhotonNetwork.LoadLevel((int)obj);
			}
		}
		else if (obj is string && UnityEngine.SceneManagement.SceneManager.GetActiveScene().name != (string)obj)
		{
	...
	protected internal void SetLevelInPropsIfSynced(object levelId)
	{
		...
		{
			object obj = PhotonNetwork.room.customProperties["curScn"];
			if ((obj is int && UnityEngine.SceneManagement.SceneManager.GetActiveScene().buildIndex == (int)obj) || (obj is string && UnityEngine.SceneManagement.SceneManager.GetActiveScene().name.Equals((string)obj)))
			{
				return;
			}
		}
