...
public class UIProgressBar : UIWidgetContainer
{
	...
	protected void Start()
	{
		...
		if (Application.isPlaying)
		{
			if (mFG == null)
			{
				UnityEngine.Debug.LogWarning("Progress bar needs a foreground widget to work with", this);
				base.enabled = false;
				return;
			}
			if (mBG != null)
	...
	protected virtual float LocalToValue(Vector2 localPos)
	{
		if (mFG != null)
		{
			...
			if (isHorizontal)
			{
				...
				return Mathf.Clamp01((!isInverted) ? num : (1f - num));
			}
			float num2 = (localPos.y - localCorners[0].y) / vector.y;
			return Mathf.Clamp01((!isInverted) ? num2 : (1f - num2));
		}
		return value;
	}
